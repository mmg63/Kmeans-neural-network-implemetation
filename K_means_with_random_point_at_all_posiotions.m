...
samples = rand(number_of_samples,2);
a = 0;
b = 4;
...
iteration = 0;
clusters_new_temp = zeros(number_of_clusters,2);
while (clusters_new_temp ~= clusters)
    iteration = iteration +1;
    if bln == 1
        clusters = clusters_new_temp;
    end
...
% define temporary matrix for seperate cluster data points
    ...
    for n = 1: number_of_samples
     ...
     r_nk(n,m_idx) = 1;
     % split data points for coloring in plot
     % this is an arbitrary work for better understanding
     if m_idx == 1
         index = index + 1;        
    ...
    for cluster_update = 1: number_of_clusters
        clusters_new_temp(cluster_update,:) = ...
